...
class MainActivity : Activity(), DJICodecManager.YuvDataCallback {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val secondAcatButton = findViewById<Button>(R.id.activity_predict_image)
            secondAcatButton.setOnClickListener(){
                val Intent = Intent(this, PredictImage::class.java)
...
*/
    private fun screenShot(buf: ByteArray, width: Int, height: Int) {
        ...
        saveImageToGallery(yuvImage, width, height)
    }

    private fun saveImageToGallery(yuvImage: YuvImage, width: Int, height: Int) {
        val resolver = applicationContext.contentResolver
        val contentValues = ContentValues().apply {
            put(MediaStore.MediaColumns.DISPLAY_NAME, "ScreenShot_" + System.currentTimeMillis() + ".jpg")
            put(MediaStore.MediaColumns.MIME_TYPE, "image/jpeg")
            put(MediaStore.MediaColumns.RELATIVE_PATH, Environment.DIRECTORY_PICTURES + File.separator + "DJI Screenshots")
        }
        ...
        uri?.let {
            ...
            runOnUiThread { displayPath(it.toString()) }
        }
    }
